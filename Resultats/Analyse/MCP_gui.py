...
def CreateTH1D(input_file):
    df = ROOT.RDataFrame("Optical", root_file)
    h = df.Histo1D(("name", "title", 100, -2, 2), "MomentumX")
    h_value = h.GetValue()
    ...
    # print(Entries)
    # HIST = TH1
    # Tree.Draw("IncidentE >> HIST", "")
    return h_value

def DisplayTH1D(Hist, ax, color=None, label=None, title=None, xlabel=None, ylabel=None, xlim=None, ylim=None, xtick=None, ytick=None, ylog=None, xlog=None, rebin=None, normalized=None, scaler=None, visible=True):
        if rebin   != None: Hist.Rebin(rebin)
        if normalized == True: integral = Hist.Integral()
        else : integral = 1.
        if scaler != None: integral = scaler
        ...
        if xlim     == None: xlim = ( bin_centers_x.min()-1, bin_centers_x.max() )
        ...
        step, = ax.step(bin_centers_x, hist_data, label=label, color=color, visible = visible)
        ax.set_xlabel(xlabel)
        ax.set_ylabel(ylabel)
        ax.set_title(title)
        #ax.set_xlim(xlim)
...
if __name__ == '__main__':
    ...
    HIST = DisplayTH1D(Plot, axs[0,0], title = "MCP data")
